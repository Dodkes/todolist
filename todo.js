...
let	inField = document.getElementById('inField'); //gets input element

toDoButton.addEventListener('click', function(){ //used variable for click event
  var paragraph = document.createElement('li'); //create element <li>
  var inputValue = document.getElementById('inField').value; //gets the input value
  if(inputValue !== "" && inputValue !== "dick" && inputValue !== "fuck" && inputValue !== "bitch"){
  ...
  inField.value = ""; //resets input field after click
	...
	}else if (inputValue == "dick" || inputValue == "fuck" || inputValue == "bitch"){ //if forbidden words
	alert("Cooldown you" + " " +inputValue + " :)");
	
	}else{ //if no value entered
